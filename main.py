...
async def run_full_workflow(topic):
    ...
    research_result = await researchgraph.ainvoke(
        {"topic": topic}, config={"callbacks": [research_handler]}
    )
    ...
    usecase_result = await usecasegraph.ainvoke(
        {"research_content": research_info}, config={"callbacks": [usecase_handler]}
    )
    ...
    resource_result = await resourcegraph.ainvoke(
        {"use_cases": usecase_info}, config={"callbacks": [resource_handler]}
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Run the agent workflow with a specified topic."
    )
    parser.add_argument(
        "--topic", type=str, required=True, help="The topic to run the workflow on."
    )
    ...
    print(
        f"Message history saved to file. Total messages: Research={len(result['message_history']['research_agent'])}, Usecase={len(result['message_history']['usecase_agent'])}, Resource={len(result['message_history']['resource_agent'])}"
    )
