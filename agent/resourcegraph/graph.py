...
from agent.resourcegraph import prompts
from agent.resourcegraph.configuration import Configuration
from agent.resourcegraph.state import InputState, OutputState, State
from agent.resourcegraph.tools import scrape_website, search, store_data
from agent.resourcegraph.utils import init_model
...
async def call_agent_model(
    ...
    # Format the prompt defined in prompts.py with the extraction schema and use_cases
    p = configuration.prompt.format(
        info=json.dumps(configuration.extraction_schema, indent=2), use_cases=state.use_cases
    )
    ...
    model = raw_model.bind_tools([scrape_website, store_data, search, store_data,  info_tool], tool_choice="any")
...
resource_workflow = StateGraph(
    State, input=InputState, output=OutputState, config_schema=Configuration
)
resource_workflow.add_node(call_agent_model)
resource_workflow.add_node(reflect)
resource_workflow.add_node("tools", ToolNode([search, scrape_website, store_data]))
resource_workflow.add_edge("__start__", "call_agent_model")
resource_workflow.add_conditional_edges("call_agent_model", route_after_agent)
resource_workflow.add_edge("tools", "call_agent_model")
resource_workflow.add_conditional_edges("reflect", route_after_checker)

resourcegraph = resource_workflow.compile()
resourcegraph.name = "Resource Collection Agent"
